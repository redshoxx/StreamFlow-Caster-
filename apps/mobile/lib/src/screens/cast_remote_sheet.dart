...
Future<void> showCastRemoteSheet(BuildContext context, StreamFlowController controller) async {
  ...
  await showModalBottomSheet<void>(
    ...
    builder: (_) => FractionallySizedBox(
      heightFactor: 0.82,
      child: CastRemoteSheet(controller: controller, device: device, media: media),
    ),
  );
}
...
class _CastRemoteSheetState extends State<CastRemoteSheet> {
  ...
  void initState() {
    super.initState();
    _refresh();
    _timer = Timer.periodic(const Duration(seconds: 1), (_) => _refresh(silent: true));
  }
  ...
  Future<void> _refresh({bool silent = false}) async {
    try {
      final status = await _client.status(widget.device);
      if (!mounted) return;
      setState(() {
        _loading = false;
        _connected = true;
        ...
        _volume = ((status['volume'] as num?)?.toDouble() ?? _volume).clamp(0.0, 1.0).toDouble();
      ...
      setState(() {
        _loading = false;
        _connected = false;
        if (!silent) _error = 'Der Fernseher ist derzeit nicht erreichbar.';
      });
    }
  }
  ...
  Future<void> _run(Future<void> Function() command) async {
    try {
      ...
      if (mounted) setState(() => _error = 'Befehl konnte nicht an den Fernseher gesendet werden.');
    }
  }
  ...
  Future<void> _seekRelative(int seconds) async {
    final target = (_positionMs + seconds * 1000)
        ...
        .toInt();
    await _run(() => _client.seek(widget.device, Duration(milliseconds: target)));
  }

  Future<void> _stop() async {
    await _run(() => _client.stop(widget.device));
      ...
      child: Column(
        ...
        children: [
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(widget.device.name, style: theme.textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w700)),
                    Text(_connected ? 'Verbunden' : 'Verbindung unterbrochen', style: theme.textTheme.bodySmall?.copyWith(color: _connected ? colors.primary : colors.error)),
                  ],
                ),
              ),
          ...
          Container(
            ...
            child: Center(child: Icon(Icons.play_circle_fill_rounded, size: 72, color: colors.onPrimaryContainer)),
          ),
          ...
          Text(widget.media.url.host, textAlign: TextAlign.center, style: theme.textTheme.bodySmall),
          if (_loading) ...[
          ...
          Slider(
            ...
            onChanged: _durationMs <= 0 ? null : (value) => setState(() => _positionMs = value.toInt()),
            onChangeEnd: _durationMs <= 0
                ? null
                : (value) => _run(() => _client.seek(widget.device, Duration(milliseconds: value.toInt()))),
          ),
          Padding(
            ...
            child: Row(
              ...
              children: [
                ...
                Text(_durationMs > 0 ? _format(_durationMs) : '--:--', style: theme.textTheme.bodySmall),
              ],
            ),
          ),
          ...
          Row(
            ...
            children: [
              IconButton(onPressed: _connected ? () => _seekRelative(-10) : null, iconSize: 34, icon: const Icon(Icons.replay_10_rounded)),
              const SizedBox(width: 18),
              FilledButton(
                onPressed: _connected
                    ? () => _run(() => _playing ? _client.pause(widget.device) : _client.play(widget.device))
                    : null,
              ...
              const SizedBox(width: 18),
              IconButton(onPressed: _connected ? () => _seekRelative(10) : null, iconSize: 34, icon: const Icon(Icons.forward_10_rounded)),
            ],
          ),
            ...
            children: [
              ...
              Expanded(
                child: Slider(
                  ...
                  onChangeEnd: _connected ? (value) => _run(() => _client.setVolume(widget.device, value)) : null,
                ),
              ),
